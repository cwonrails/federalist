...
const listFolders = path => new Promise((resolve, reject) => {
  s3Client.listObjects({
    ...
    if (err) {
      ...
      prefixes = data.CommonPrefixes.map(prefix => prefix.Prefix.split('/').slice(-2)[0]);
      resolve(prefixes);
    }
  }
  );
...
const listFiles = path => new Promise((resolve, reject) => {
  s3Client.listObjects({
    Bucket: s3Config.bucket,
    Prefix: path,
  }, (err, data) => {
